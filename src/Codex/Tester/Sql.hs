...
sqlSelectTester =
  ...
  ---
    ...
    confArgs <-
      getOptConfArgs
        "language.sql.select.args"
        [("-H", "host"), ("-P", "port"), ("-u", "user"), ("-p", "pass")]
    metaArgs <- getOptMetaArgs [("-d", "db-name")]
...
sqlEditTester =
  ...
  ---
    ...
    metaArgs <- getOptMetaArgs [("-i", "db-init-sql"), ("-I", "db-init-file")]
    answer <- liftIO (getSqlAnswer meta)
    withTemp "submit.sql" src $ \submittedFilePath -> do
      chmod readable submittedFilePath
      classify <$>
...
getOptConfArgs prefix opts = concat <$> mapM optConfArg opts
  where
...
getSqlAnswer meta = do
  case lookupFromMeta "answer-sql" meta of
    Nothing -> throwIO (miscError "no sql-answer specified in metadata")
